...
pipeline TessellationPipeline : StandardPipeline
{
    ...
    [InvocationId]
    extern @CoarseVertex int threadId;
    extern @CoarseVertex PackedBuffer<MeshVertex> vertAttribs;
    import(MeshVertex->CoarseVertex) vertexImport() { return vertAttribs[threadId]; }
    
    // implicit import operator CoarseVertex->CornerPoint
    extern @CornerPoint PackedBuffer<CoarseVertex> CoarseVertex_ControlPoint;
    [PerCornerIterator]
    extern @CornerPoint int sysLocalIterator;
    import (CoarseVertex->CornerPoint) standardImport()
        require trait IsTriviallyPassable(CoarseVertex)
    ...
    extern @ControlPoint PackedBuffer<CoarseVertex> CoarseVertex_ControlPoint;
    extern @TessPatch PackedBuffer<CoarseVertex> CoarseVertex_ControlPoint;
    [InvocationId]
    extern @ControlPoint int invocationId;
    extern @(ControlPoint, TessPatch) Uniform<int[]> indexBuffer;
    import(CoarseVertex->ControlPoint) indexImport(int id)
        require trait IsTriviallyPassable(CoarseVertex)
    {
        return CoarseVertex_ControlPoint[indexBuffer[invocationId/3 + id]];
    }
    import(CoarseVertex->TessPatch) indexImport(int id)
        require trait IsTriviallyPassable(CoarseVertex)
    {
    ...
    [InvocationId]
    extern @FineVertex int invocationId;
    extern @FineVertex PackedBuffer<ControlPoint> ControlPoint_tes;
    import(ControlPoint->FineVertex) indexImport(int id)
        require trait IsTriviallyPassable(CoarseVertex)
    {
        return ControlPoint_tes[id];
