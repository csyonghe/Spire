...
namespace GraphicsUI
{
	...
	private:
		...
		const char * uberSpireShader = R"(
			pipeline EnginePipeline
			{
				...
				world vs : "BACKEND_NAME(vertex:projCoord) " using projCoord export standardExport;
				world fs : "BACKEND_NAME" export fragmentExport;

				require @vs vec4 projCoord;

				import uniformImport(uniformIn->vs);
				import uniformImport(uniformIn->fs);

				import uniformImport(primitiveUniform->vs);
				import uniformImport(primitiveUniform->fs);

				import uniformImport(textBuffer->vs);
				import uniformImport(textBuffer->fs);
	...
	public:
		GLUIRenderer(UIWindowsSystemInterface * pSystem, GameEngine::HardwareRenderer * hw)
		{
			...
			auto compiledUIShader = SpireLib::CompileShaderSource(compileResult, spireShaderSrc, options).First();

			if (rendererApi->GetSpireBackendName() == L"glsl")
			{
				auto vsCode = compiledUIShader.Sources[L"vs"]().GetAllCodeGLSL();
				auto fsCode = compiledUIShader.Sources[L"fs"]().GetAllCodeGLSL();
		...
		{
			rendererApi->Blit(uiOverlayTexture.Ptr(), baseTexture);
