...
namespace GameEngine
{
	...
	public:
		...
		virtual void RenderFrame() override
		{
			...
			//List<char> data;
			//data.SetSize(screenWidth * screenHeight * 4);
			//colorTexture->GetData(0, data.Buffer(), data.Count());

			//auto writePPM = [](String fileName, char* buffer, int width, int height) -> void
			//{
			//	FILE * f;
			...
			//			fwrite((void*)&buffer[4 * ((height - h - 1)*width + w)], sizeof(char), 3, f);
			//	fclose(f);
			//};

			//writePPM("debug.ppm", data.Buffer(), screenWidth, screenHeight);
		}
