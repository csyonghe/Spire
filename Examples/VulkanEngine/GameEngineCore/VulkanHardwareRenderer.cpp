...
namespace VK
{
	...
	//		vk::ResultValue<vk::DescriptorPool> wat = Device().createDescriptorPool(poolCreateInfo);
	//	}
	//	~DescriptorPoolObject()
	//	{
	...
	*/
	...
	private:
		...
		static void CreateCommandPool()
		{
			...
			State().primaryBuffer = RendererState::CreateCommandBuffer(RendererState::RenderCommandPool());
		}
	...
	public:
		...
		static const vk::CommandBuffer& PrimaryBuffer()
		{
			return State().primaryBuffer;
		}
	...
	*/
	...
	public:
		...
		~PipelineInstance()
		{
			...
			if (descriptorPool) RendererState::Device().destroyDescriptorPool(descriptorPool);
		}
		...
		pipelineBindings(pipelineBinding)
	{
		if (pipeline->descriptorPoolSizes.Count() > 0)
		{
			...
			vk::DescriptorPoolCreateInfo descriptorPoolCreateInfo = vk::DescriptorPoolCreateInfo()
				...
				.setMaxSets(1)
				.setPoolSizeCount(pipeline->descriptorPoolSizes.Count())
				.setPPoolSizes(pipeline->descriptorPoolSizes.Buffer());

			this->descriptorPool = RendererState::Device().createDescriptorPool(descriptorPoolCreateInfo).value;

			// Create Descriptor Set
			vk::DescriptorSetAllocateInfo descriptorSetAllocateInfo = vk::DescriptorSetAllocateInfo()
				.setDescriptorPool(descriptorPool)
				.setDescriptorSetCount(pipeline->descriptorSetLayouts.Count())
				.setPSetLayouts(pipeline->descriptorSetLayouts.Buffer());

			this->descriptorSet = RendererState::Device().allocateDescriptorSets(descriptorSetAllocateInfo).value[0];
	...
	public:
		...
		inline void BeginRecording(GameEngine::RenderTargetLayout* renderTargetLayout, vk::Framebuffer framebuffer)
		{
			...
			//TODO: Each command buffer should have a fence so that we can make sure it's not in use
	...
	public:
		...
		virtual void ClearTexture(GameEngine::Texture2D* texture) override
		{
			...
			RendererState::PrimaryBuffer().begin(primaryBeginInfo);

			switch (dynamic_cast<VK::Texture2D*>(texture)->currentLayout)
			{
			case vk::ImageLayout::eColorAttachmentOptimal:
				RendererState::PrimaryBuffer().clearColorImage(
				...
				break;
			case vk::ImageLayout::eDepthStencilAttachmentOptimal:
				RendererState::PrimaryBuffer().clearDepthStencilImage(
					dynamic_cast<VK::Texture2D*>(texture)->image,
					vk::ImageLayout::eGeneral,
			...
			RendererState::PrimaryBuffer().end();
			...
			vk::SubmitInfo submitInfo = vk::SubmitInfo()
				...
				.setPWaitDstStageMask(nullptr)
				.setCommandBufferCount(1)
				.setPCommandBuffers(&RendererState::PrimaryBuffer())
				.setSignalSemaphoreCount(0)
				.setPSignalSemaphores(nullptr);
		...
		{
			//TODO: Code currently uses a single global primary command buffer
			//      this means that we have to wait until submit is finished before recording again
			//      but currently unsure of a way to remedy this cleanly without creating a command
			//      buffer on every single call to execute
			//
			//      Could maybe make a shared pool of command buffers that are acquired using 
			//      fences/semaphores to signal their state. Take any buffer that is completed.

			//TEMP
			//TODO: see note at top of function
			RendererState::RenderQueue().waitIdle();
			// Moved from the bottom of the function to here
			//ENDTEMP
			...
			RendererState::PrimaryBuffer().begin(primaryBeginInfo);
			RendererState::PrimaryBuffer().beginRenderPass(renderPassBeginInfo, vk::SubpassContents::eSecondaryCommandBuffers);

			RendererState::PrimaryBuffer().executeCommands(commandBuffersToExecute.Count(), commandBuffersToExecute.Buffer());

			RendererState::PrimaryBuffer().endRenderPass();
			RendererState::PrimaryBuffer().end();
